...
- `payload/` 是分发源:`rules.md`(AGENTS.md 协议块与路由表)、`STATE.md`(知识模板)、
  `skills/tidykeep/`(单一 skill,内含四模块)。
- **`tidykeep` skill(统一入口)** 整合原四个独立 skill 为单一入口,内部分四模块:
  知识收尾(原 tidykeep,§1)、测试真实性(原 blind-test,§2)、开发流程(原 sdlc,§3)、
  代码审计(原 rigor3,§4)。主文件 SKILL.md 425 行,通过章节编号(§1-§4)实现内部路由。
  26 个 references 按模块分组到 core/test/sdlc/audit 子目录,保持原有内容不变。
  description 覆盖所有原触发场景,路由表写进规则文件作为兜底。
- **§1 知识收尾模块**:六事实面完成合同(代码/运行态/文档/规则/记忆/工作区),
  轻量/完整双路径,STATE.md 同步,文档漂移核对。4 个 references 在 core/。
- **§2 测试真实性模块**:契约先行、作者与实现者隔离(工具权限)、验收条款一一对应、
  变异检查机械证明。1 个 reference 在 test/。
- **§3 开发流程模块**:需求澄清→价值评估→方案设计→PRD→开发→质量验证→修复回归→
  文档维护八阶段,强制按序执行。10 个 references 在 sdlc/。
- **§4 代码审计模块**:三维度(代码/架构/工程卫生)60 控制项,临时生成引擎+一致性校验,
  最低维度决定结论。上游 MaySudo/rigor3 v0.2.0(MIT)。11 个 references 在 audit/。
...
| D-032 | 2026-09-17 | **合并四 skill 为单一 tidykeep**:用户反馈安装时一次出现 4 个 skill 太臃肿。合并策略:单一 SKILL.md 入口(425 行,符合 <500 行标准)+ 四模块章节(§1-§4)+ references 分组子目录(core/test/sdlc/audit)。description 采用"pushy"风格覆盖所有原触发场景,内部路由表明确模块间转移规则,规则文件路由表改为指向 §1-§4。按 Claude 官方 skill-creator 标准:渐进式披露(概览→详细→references)、解释 why 而非强制 MUST、保持主文件精简。rigor3 上游信息(MIT v0.2.0)保留在 §4 开头。**已知代价**:单一 description 语义匹配可能不如 4 个专项精准(但有路由表兜底);合并后用户无法单独安装某一模块(但可通过 §X 章节选择性使用)。**优势**:用户体验简化为"一个 skill 解决所有事",安装卸载只需操作一个目录,符合个人 skill 库"换机器一条命令搞定"的初衷 | active |
...
| 2026-09-17 | **独立的 tidykeep/blind-test/sdlc/rigor3 四个 skill 目录**(共 834 行 SKILL.md + 26 个 references) | 用户反馈安装时一次出现 4 个 skill 太臃肿,希望简化为单一 skill | 合并为统一的 tidykeep skill(425 行 SKILL.md),内部模块化为 §1-§4 四章节,references 分组到 core/test/sdlc/audit 子目录(D-032) |
