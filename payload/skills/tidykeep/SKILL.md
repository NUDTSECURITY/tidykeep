...
name: tidykeep
description: >-
  知识与规范收尾:让代码、真实运行态、项目文档、Agent 规则、获准维护的记忆和工作区状态
  彼此一致,使下一次会话或第一次接手的人只找到唯一现役答案。当用户提到 tidykeep、洁癖、
  初始化扫描、考古、建台账、收尾同步、体检审计、清理冗余代码、删除旧版本脚本、STATE、
  文档过期、文档与代码对不上、把文档和记忆整理一下、交接给同事或新会话、审一下规则有没有
  被遵守时,必须使用本技能。每次开发任务准备收尾、需要更新 STATE.md 时也应使用。
  不要用于:纯编码/重构/调试、整理数据或散文(JSON、周报、changelog 文案)、
  以及没有项目知识语境的一句「整理」。
---
...
| 交给谁 | 什么事 |
|---|---|
| **tidykeep**(本 skill) | 项目知识收尾:`STATE.md`、规则文件、Agent 记忆、工作区残留、文档漂移核对、提交规范、对外动作边界 |
| `sdlc` | 需求→PRD→开发→质量→交付的八阶段长流程,以及**技术文档本身**(API/模块/架构说明的生成与增量更新) |
| `blind-test` | **设计与编写测试**:契约先行、测试作者与实现者隔离、验收条款配额、变异检查 |

收尾时若发现测试缺失或可疑,**不要自己补测试**——转 `blind-test`,否则同一上下文写出来的
就是实现的镜像。若本次收尾发生在 sdlc 流程中途,先完成当前阶段再收尾,不要打断阶段序列。
