...
description: >-
  ...
  初始化扫描、考古、收尾同步、STATE、文档过期、文档与代码对不上、把文档和记忆整理一下、
  交接给同事或新会话、审一下规则有没有被遵守、清点会话残留(一次性计划文档、调试脚本、
  被替代的旧副本)时,必须使用本技能。每次开发任务准备收尾、需要更新 STATE.md 时也应使用。
  不要用于:纯编码/重构/调试、整理数据或散文(JSON、周报、changelog 文案)、
  以及没有项目知识语境的一句「整理」;**代码层的质量审计与技术债盘点(重复实现、死代码、
  耦合、依赖、错误处理、架构边界)归 rigor3**,本技能只管知识层。
---
